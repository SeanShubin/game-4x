...
data class Result(
    ...
    companion object {
        ...
        fun missingParameter(state: Items, index: Int): Result = failure(
            state, listOf(
                "Missing parameter ${index + 1}"
            )
        )

        fun invalidSyntax(state: Items, line: String): Result = failure(
            state, listOf(
                "Invalid syntax for '$line'"
            )
        )

        fun commandNotFound(state: Items, name: String): Result = failure(
            state, listOf(
