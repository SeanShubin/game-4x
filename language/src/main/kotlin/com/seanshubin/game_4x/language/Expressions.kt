...
object Expressions {
    ...
    val trueValue = StringOf("true", "true")
    val falseValue = StringOf("false", "false")
    val booleanValue = OneOf("boolean", trueValue, falseValue)
    val number = OneOrMore("number", numberChar)
    val primitive = OneOf("primitive", alias, number, booleanValue, name)
    ...
    val attributeList = ZeroOrMore("attribute-list", attribute)
    val closeBrace = CharOf("close-brace", '}')
    val unnamed = SeqOf("unnamed", openBrace, attributeList, closeBrace)
    val named = SeqOf("named", openBrace, name, attributeList, closeBrace)
    val item = OneOf("item", named, unnamed)
    val parameter = OneOf("parameter", item, primitive)
    val parameterList = ZeroOrMoreInterleave("parameter-list", parameter, maybeWhitespace)
